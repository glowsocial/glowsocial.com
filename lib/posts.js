...
export function getRelatedPosts(currentSlug, count = 3) {
  const cacheKey = `${currentSlug}:${count}`;
  const cached = relatedPostsCache.get(cacheKey);
  if (USE_CONTENT_CACHE && cached) return cached;

  const index = getBlogKeywordIndex();
  const current = index.find(({ post }) => post.slug === currentSlug);
  if (!current) return [];
  ...
  const related = index
    ...
    .map(({ post, keywords }) => {
      ...
      const score =
        keywords.length > 0 ? matches / Math.sqrt(keywords.length) : 0;
  ...
  if (USE_CONTENT_CACHE) relatedPostsCache.set(cacheKey, related);
  return related;
}
...
export function getQuestionBySlug(slug) {
  ...
  for (const ext of [".md", ".mdx"]) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      // Extract Direct Answer section content as description
      let description = data.description;
      if (!description) {
        const match = content.match(/## Direct Answer\r?\n([\s\S]*?)(?=\r?\n##|$)/);
        description = match ? match[1].trim().replace(/\r?\n/g, " ") : "";
